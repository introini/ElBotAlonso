...
def goal_stats_raw(url):
    ...
    goals = []
    for item in table:
        player = item.find('div', attrs={'class': 'ListItem-text-value'}).find('div', attrs={'class': 'ListItem-text'}).text
        team = item.find('div', attrs={'class': 'ListItem-text-value'}).find('div', attrs={'class': 'ListItem-value'}).text
